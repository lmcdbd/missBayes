...
runModel <- function(contrast_data, sigma_p2p, sigma_jp2p, zS, sigma_1, default_data, 
  ...
  if (parallel){
    ...
    if (.Platform$OS.type == "unix") {
      ...
      clusterExport(cl, varlist = c("fixed_data", "model_txt", "model_logit_txt",
                                    "process_row", "ROPE", "threshold", "mcmcDiag",
                                    "f_alpha", "f_beta", "summarizePost", 
                                    "generate_overdispersed_inits",
                                    "jags.model", "update", "coda.samples",
                                    "effectiveSize", "gelman.diag", "contrast_data", "default_data",
                                    "burn.in", "n.adapt", "n.chains", "n.iter", "parallel", "sigma_1",
                                    "sigma_jp2p", "sigma_p2p", "zS", "var", "mean", "ifelse", "is.na"), envir = environment())
...
cat("Total runtime:", format(finish.time - start.time))
