...
# 2.9 process each row ------
process_row <- function(i, fixed_data, model_txt, model_logit_txt) {
  ...
  alpha1 <- tryCatch(f_alpha(sample_mean_1, alpha, a, b), error = function(e) NA)
  beta1 <- tryCatch(f_beta(sample_mean_1, beta, a, b), error = function(e) NA)
  alpha2 <- tryCatch(f_alpha(sample_mean_2, alpha, a, b), error = function(e) NA)
  beta2 <- tryCatch(f_beta(sample_mean_2, beta, a, b), error = function(e) NA)
...
runModel <- function(overall_distri, contrast_data, sigma_p2p, sigma_jp2p, zS, sigma_1, default_data, 
                     n.chains, burn.in, n.iter, n.adapt, parallel){
...
BayesMissingModel <-  function(values, groups, comparisons, filter4NAs = FALSE, threshold = 0, parallel = TRUE, ROPE = c(-0.2,0.2), 
  ...
  for (contrast in colnames(comparisons)){
    ...
    bayesResults[[contrast]] <- runModel(values, cd, s2P, sjp2P, zS, s1, d, n.chains, burn.in, n.iter, n.adapt, parallel)
  }
...
comparison <- limma::makeContrasts(G70_10_20 - G70_20_10, levels = levels(group))
start.time <- Sys.time()
output <- BayesMissingModel(overall_distri, group, comparison, filter4NAs = TRUE)
